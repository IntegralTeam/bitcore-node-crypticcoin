...
var CrypticCoinRPC = require('crypticcoind-rpc');
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['listMasternodes', this, this.listMasternodes, 0],
  ];
  return methods;
};
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in crypticcoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in crypticcoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in crypticcoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './crypticcoin.conf');

  log.info('Using CrypticCoin config file:', configPath);
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to crypticcoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from crypticcoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from crypticcoind. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in crypticcoin.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that crypticcoind is undergoing a reindex. ' +
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('CrypticCoin Daemon Ready');
          callback();
        });
      });
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/crypticcoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/crypticcoin.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('CrypticCoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('CrypticCoin Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/crypticcoind.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned CrypticCoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean CrypticCoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting CrypticCoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('CrypticCoin process unexpectedly exited with code:', code);
        log.warn('Restarting CrypticCoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('CrypticCoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new CrypticCoinRPC({
        protocol: 'http',
        host: '127.0.0.1',
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn crypticcoind.'));
      }
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to crypticcoind.'));
    }
...
Bitcoin.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('CrypticCoin configuration options "spawn" or "connect" are expected'));
    }
...
Bitcoin.prototype.getRawBlock = function(blockArg, callback) {
  ...
  function queryBlock(err, blockhash) {
    ...
    self._tryAllClients(function(client, done) {
      self.client.getBlock(blockhash, 0, function(err, response) {
        if (err) {
          return done(self._wrapRPCError(err));
        }
...
Bitcoin.prototype.getBlockOverview = function(blockArg, callback) {
  ...
  function queryBlock(err, blockhash) {
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, 1, function(err, response) {
          if (err) {
            return done(self._wrapRPCError(err));
          }
...
Bitcoin.prototype.getBlock = function(blockArg, callback) {
  ...
  function queryBlock(err, blockhash) {
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, 0, function(err, response) {
          if (err) {
            return done(self._wrapRPCError(err));
          }
...
Bitcoin.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
	  ...
	  tx.fInstant = result.dpos_instant;
        }
        // Sapling END
...
/**
 * Will retrieve a masternodes list
 * @param {Function} callback
 */
Bitcoin.prototype.listMasternodes = function(callback) {
  var self = this;
  this.client.mn_list([], function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    callback(null, response.result);
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('crypticcoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('crypticcoind process did not exit'));
      }
